...
def train_APT(
    ...
    for r in range(1, R + 1):
        ...
        if has_MoG:
            ...
            alpha_prop, mu_prop, Sigma_prop_inv, Sigma_prop_det = cde.density_estimator._get_MoG_params(
                params
            )
        ...
        if r == 1:
            ...
            if has_MoG:
                ...
                Sigma_prop_inv_all = torch.cat(
                    (Sigma_prop_inv_all, Sigma_prop_inv), dim=0
                )
                Sigma_prop_det_all = torch.cat(
                    (Sigma_prop_det_all, Sigma_prop_det), dim=0
                )
        ...
        for i in range(1, num_iters + 1):
            ...
            if has_MoG:
                if r == 1:
                    ...
                    params = cde.param_net(x)
                    alpha, mu, Sigma_inv, Sigma_det = cde.density_estimator._get_MoG_params(
                        params
                    )
            ...
            #for _i, param in enumerate(cde.parameters()):
            #    dbg_check(param, 'param %d' % _i)
            #for _i, param in enumerate(cde.parameters()):
            #    dbg_check(param.grad, 'param grad %d' % _i)
        ...
        np.savez(
            ...
            it_time=np.mean(np.array(it_time)),
...
def MoG_proposal_posterior(
    ...
    Sigma_star_det = 1.0 / torch.det(Sigma_star_inv)
    ...
    gauss_probs_num = torch.exp(
        -0.5 * torch.matmul(torch.matmul(z_mu_T, Sigma_star_inv), z_mu)
    )[:, :, :, 0, 0]
